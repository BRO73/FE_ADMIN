...
} from "@/realtime/sources/IKitchenSource";
import {
...
export function useKitchen({ intervalMs = 5000 }: UseKitchenOpts = {}) {
  ...
  const [tickets, setTickets] = useState<TicketsShape>({
    pending: [],
    inProgress: [],
    ready: [],
    readyToServe: [],
  ...
  const updateServerOffset = useCallback(
    (serverIso: string) => {
  ...
  const handleEvent = useCallback(
    (evt: KitchenRealtimeEvent) => {
      ...
      if (
        evt.payload.serverTime &&
        evt.payload.serverTime === lastServerTimeRef.current
      )
        return;
  ...
  /* ====================================================================
    ...
    Xóa 'await refresh()' và 'refresh' khỏi dependency array.
    Backend sẽ tự động gửi snapshot qua WebSocket sau khi update,
    nên không cần gọi lại API GET.
    ====================================================================
    */

  const updateStatus = useCallback(
    async (orderDetailId: number, status: OrderItemStatus): Promise<void> => {
      await apiUpdateStatus(orderDetailId, status);
      // await refresh(); // <-- ĐÃ XÓA
    },
    []
  ); // <-- ĐÃ XÓA 'refresh'

  const completeOneUnit = useCallback(
    async (orderDetailId: number): Promise<void> => {
